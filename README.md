...
<details>
<summary><b>3. 관제 결과를 정확하게 판단하기 위해 함께 확인한 것</b></summary>

서버 상태는 한 가지만 확인하면 정상으로 잘못 판단할 수 있습니다. 예를 들어 앱 이름이 보여도 실제로 접속을 받지 못할 수 있고, 잠깐 측정한 자원 사용량은 실제 상태와 다를 수 있습니다. 그래서 다음 항목을 함께 확인했습니다.

- <b>앱 상태:</b> 실제 앱이 실행 중인지 확인하고, 서비스 포트에서 접속도 받고 있는지 함께 검사합니다.
- <b>서버 자원:</b> 순간적인 수치에만 의존하지 않고 CPU 사용량을 일정 시간 측정합니다. 메모리는 필요할 때 비울 수 있는 캐시를 제외해 실제 사용량에 가깝게 계산합니다.
- <b>방화벽:</b> 방화벽 설정과 서비스 실행 상태를 함께 확인해 실제로 보호 기능이 작동 중인지 판단합니다.
- <b>경고 기준:</b> CPU·메모리·디스크 사용률이 설정한 기준을 넘었을 때만 경고를 남깁니다.
