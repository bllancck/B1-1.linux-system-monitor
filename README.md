...
cron은 로그인 환경을 거치지 않아 앱 경로와 포트 설정을 자동으로 불러오지 못합니다. 따라서 `monitor.sh`가 환경 파일을 직접 읽어, 수동 실행과 cron 실행이 같은 설정을 사용하도록 했습니다.

테스트에서는 운영 설정을 건드리지 않고 로그 위치나 경고 기준만 임시로 바꿀 수 있습니다. 설정은 <b>실행 시 전달한 값 → 환경 파일 → 스크립트 기본값</b> 순으로 적용됩니다. 예를 들어 다음 명령은 임시 로그 디렉토리에서 경고와 로그 정리 기능을 확인합니다.
