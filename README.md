...
<details>
<summary><b>3. 관제 상태를 직접 확인한 이유</b></summary>

관제 스크립트는 앱이 정상적으로 실행 중인지, 서버 자원이 기준을 넘었는지를 스스로 판단해야 합니다. 명령 하나의 결과만 그대로 사용하면 잘못 판단할 수 있어 다음과 같이 확인했습니다.

- <b>앱 프로세스:</b> 앱을 실행할 때 함께 사용된 `su`와 `bash`까지 앱으로 오인하지 않도록, 이름이 정확히 일치하는 프로세스만 찾습니다. 앱 프로세스가 두 개라면 실행을 시작한 부모 프로세스의 PID를 대표값으로 기록합니다.
- <b>CPU:</b> 한순간의 값은 쉽게 흔들리므로 CPU 상태를 1초 간격으로 두 번 읽고, 그 사이에 실제로 사용된 비율을 계산합니다.
- <b>메모리:</b> Linux는 남는 메모리를 캐시로 활용하며 필요할 때 다시 돌려줍니다. 이 캐시를 모두 사용 중인 메모리로 보지 않도록 실제로 사용할 수 있는 메모리를 제외해 계산합니다.
- <b>방화벽:</b> cron을 실행하는 일반 계정은 관리자용 방화벽 조회 명령을 사용할 수 없습니다. 대신 방화벽 설정이 켜져 있는지와 관련 서비스가 실행 중인지를 함께 확인합니다.
- <b>임계값 비교:</b> CPU나 메모리 사용률에는 소수가 포함되므로 `15.7 > 10` 같은 값도 정확히 비교할 수 있는 `awk`를 사용합니다.

</details>

<details>
<summary><b>4. cron에서도 같은 설정을 사용하고 테스트 값을 바꿀 수 있게 한 이유</b></summary>

터미널에서 직접 실행할 때는 로그인 과정에서 앱 설정을 불러오지만, cron은 별도의 환경에서 실행되므로 이 설정을 알지 못합니다. 그래서 `monitor.sh`가 환경 파일을 직접 읽어, 수동으로 실행할 때와 cron으로 실행할 때 같은 경로와 포트를 사용하도록 했습니다.

테스트할 때는 로그 위치나 경고 기준을 잠시 바꿔야 할 수 있습니다. 이때 환경 파일의 값으로 무조건 덮어쓰지 않고 <b>실행할 때 전달한 값 → 환경 파일의 값 → 스크립트 기본값</b> 순으로 사용하도록 했습니다. 덕분에 운영 설정은 그대로 둔 채 다음처럼 임시 로그 디렉토리에서 경고와 로그 정리를 확인할 수 있습니다.
